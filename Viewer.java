...
public class Viewer {
    ...
    public static <O extends OverCell<O, U>, U extends UnderCell<U, O>, C extends Cell<C>, W extends WeaveGrid<O, U>> void main( String[] argv) throws Exception {
        if (DEBUG) {
            System.out.printf("+ Maze image viewer - %s\n", Constants.VERSION);
            System.out.printf("+ %s\n", Constants.COPYRIGHT);
        }
        ...
        if (monitor < 0 || monitor >= gds.size()) monitor = 0;
        ...
        String about = Constants.VERSION + " / " + Constants.COPYRIGHT;
        Font font = font();
        Font msg = Font.decode(Constants.MSG_FONT);
        int border = fullscreen ? 0 : (int) (font.getSize2D() / 2f);
        int height = screen.getHeight() - (fullscreen ?  0 : (font.getSize() + (5 * border)));
            ...
            public void keyTyped(KeyEvent event) {
                switch (event.getKeyChar()) {
                    ...
                    case 'Q':
                        if (quit.compareAndSet(false, true)) {
                            ...
                            wait.set(false);
                            paused.set(false);
                            screen.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
                        }
                        break;
                    case 's':
                    case 'S':
                        save.compareAndSet(false, true);
                    ...
                    case ' ':
                        ...
                        break;
                    default:
                        if (DEBUG) System.out.printf("? key '%c' not recognised\n", event.getKeyChar());
                        break;
                }
                ...
                if (!fullscreen) {
                    synchronized (lock) {
                        ...
                        g.clearRect(border, border / 4, g.getFontMetrics().stringWidth("saving quitting paused wait "), msg.getSize() + border / 4);
                        g.drawString(
                                (save.get() ? "saving " : "       ") +
                                (quit.get() ? "quitting " : "         ") +
                                (paused.get() ? "paused " : "       ") +
                                (wait.get() ? "wait " : "     "),
                                border, border / 4 + msg.getSize());
                    }
                }
            }
        });
        ...
        do {
            ...
            synchronized (lock) {
                ...
                if (!fullscreen) {
                    g.setColor(dark ? Color.DARK_GRAY : Color.LIGHT_GRAY);
            ...
            if (zoom()) {
                ...
                int steps = random(1000, 5000), total = 3 * steps;
                float zoom = ((float) Math.PI / 2f) / (2f * steps);
                float rotate = zoom * (choose() ? -1f : +1f);
                float move = rotate * 0.25f;

                for (int z = 0; !skip.get() && z < total; z++) {
                    start = Instant.now();

                    if (z > 2 * steps) {
                        if (image.getWidth() > image.getHeight()) {
                            transform.translate(image.getWidth() * Math.abs(move) * -0.5f, 0f);
                        } else {
                            transform.translate(0f, image.getHeight() * Math.abs(move) * -0.5f);
                        }
                    }
                    if (z > steps) {
                ...
                if (!skip.get()) Thread.sleep(TimeUnit.SECONDS.toMillis(minPause()));
                while (paused.get());
            }
            ...
            if (save.compareAndSet(true, false)) {
